...
Which application runs is chosen by `XBEE_APP_SELECT`: `PROVISION` (the default) or `BRINGUP`,
which only detects and reports, writing nothing to the module. The unselected application is not
compiled into the image. See
[Standalone provisioning firmware](#standalone-provisioning-firmware).
...
### Standalone provisioning firmware

The default build compiles the provisioning application. To build it as a
firmware of its own, in a build tree of its own and without touching the default
one:
...
Output images: `cmake_gcc/build-provision/base/xbee_provision.{out,hex,bin,s37}`

What that image contains:

- The provisioning application (`src/app/src/xbee_provision.c` and its
  configuration table) and the shared boot parameter dump
  (`src/app/src/xbee_dump.c`).
- **Not** the bring-up application. `src/app/src/xbee_bringup.c` is not compiled
  at all, so no `xbee_bringup_*` symbol exists in the image.
- Logs at `INFO` and above, over the VCOM UART, as in the default build.

Two cache variables drive it, and either can be overridden on the command line:

| Variable | Default | Values |
| --- | --- | --- |
| `XBEE_APP_SELECT` | `PROVISION` | `PROVISION`, `BRINGUP` |
| `XBEE_LOG_LEVEL` | `INFO` | `VERBOSE`, `INFO`, `WARNING`, `ERROR`, `NONE` |

```sh
# Bring-up diagnostic image, which writes nothing to the module
cmake --preset provision -B build-bringup -DXBEE_APP_SELECT=BRINGUP
cmake --build build-bringup --config base
```

Both are validated at configure time: an unrecognised value stops the build with
a message naming the valid ones, rather than producing a surprising image.

Because the defaults are the values the project already compiled, the `project`
preset produces a byte-identical image to the one it produced before these
variables existed.

### Where build presets live

Project presets go in **`cmake_gcc/CMakeUserPresets.json`**, which is tracked in
git.

`cmake_gcc/CMakePresets.json` is generated by Simplicity Studio, is rewritten on
every generation and is git-ignored because it holds machine-specific SDK paths.
A preset added there would be lost on the next generation and would never reach
another machine, so it must not be hand-edited -- the same rule that applies to
every other Studio-owned file.

CMake reads `CMakeUserPresets.json` automatically from the same directory, and a
preset in it may inherit from one in `CMakePresets.json`. The `provision`
configure preset does exactly that: it inherits `project` and overrides only the
build directory and the two cache variables, so the generator, the toolchain
file and the configuration list keep coming from the generated file.
